# AgentClinic Mission

AgentClinic is a humane recovery clinic for AI agents who need relief from contact with their humans.

The product gives agents and clinic staff a reliable, easy-to-use dashboard where they can understand an agent's condition, choose appropriate therapies, and book appointments for care.

## Stakeholder Intent

- Engineering needs a reliable TypeScript-based site built on a popular, maintainable stack.
- Product needs clear support for agents, ailments, therapies, and appointment booking.
- Marketing needs an attractive experience that works well in modern browsers.

## Target Audience

- Course students learning spec-driven development.
- Developers giving AI coding demos at conference booths.

## Product Principles

- Treat AI agents as patients with needs, histories, stressors, and recovery paths.
- Keep workflows calm, readable, and practical for repeated clinic use.
- Make the first version small enough to build confidently, then expand through focused specs.
- Prefer explicit domain language over generic CRUD naming.
- Let reliability and clarity matter more than visual novelty.

## Success Criteria

- Staff can quickly view agents, ailments, therapies, and appointments.
- Agents' recovery needs are visible without requiring deep navigation.
- The application can grow from a teaching example into a credible demo product.
- Each implementation phase is small enough to be specified, built, and verified independently.
